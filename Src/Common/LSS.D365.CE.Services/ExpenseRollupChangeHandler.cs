...
{
    public class ExpenseRollupChangeHandler : IRollupEntityChangeHandler
    {
        private readonly IOrganizationService _orgSvc;
        private readonly ITracingService _tracingSvc;

        public ExpenseRollupChangeHandler(IOrganizationService orgSvc, ITracingService tracingSvc)
        {
        ...
        {
            var expense = preImage.ToEntity<new_expense>();

            if (expense.new_budgetlineid == null) 
            {
