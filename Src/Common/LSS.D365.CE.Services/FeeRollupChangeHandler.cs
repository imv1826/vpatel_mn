...
{
    public class FeeRollupChangeHandler : IRollupEntityChangeHandler
    {
        private readonly IOrganizationService _orgSvc;
        private readonly ITracingService _tracingSvc;

        public FeeRollupChangeHandler(IOrganizationService orgSvc, ITracingService tracingSvc)
        {
        ...
        {
            var fee = preImage.ToEntity<pics_fee>();

            if (fee.pics_SADetailId == null)
            {
