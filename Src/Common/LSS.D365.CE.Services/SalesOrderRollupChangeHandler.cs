...
{
    public class SalesOrderRollupChangeHandler : IRollupEntityChangeHandler
    {
        private readonly IOrganizationService _orgSvc;
        private readonly ITracingService _tracingSvc;
        
        public SalesOrderRollupChangeHandler(IOrganizationService orgSvc, ITracingService tracingSvc, ChangeDetectConfig config)
        {
        ...
        {
            var salesOrder = preImage.ToEntity<SalesOrder>();
            
            if (salesOrder.new_serviceagreementdetailid == null)
            {
                _tracingSvc.Trace("Entity does not have a related service agreement detail. No recalculation needed.");
                return null;
            }

            var saDetail = _orgSvc.Retrieve(
                    new_serviceagreementdetail.EntityLogicalName,
                    salesOrder.new_serviceagreementdetailid.Id,
                    new ColumnSet(
